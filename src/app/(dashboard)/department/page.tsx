...
export default function DepartmentPage() {
  ...
  const fetchDepartments = async () => {
    try {
      ...
      console.log(err);
      setError(t("department.error"));
      toast({
        title: "Error",
        description: t("department.error"),
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.log(err);
      setError(t("department.error.delete"));
      toast({
        title: "Error",
        description: t("department.error.delete"),
        variant: "destructive",
      });
    }
  };
  ...
  const handleSave = async (department: Department) => {
    try {
      ...
      console.log(err);
      setError(t("department.error.save"));
      toast({
        title: "Error",
        description: t("department.error.save"),
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="w-full p-4">
      <div className="">
        <h1 className="text-primary-heading">{t("department.title")}</h1>
        <p className="text-gray-500 text-md">{t("department.subtitle")}</p>
      </div>

      <Button onClick={handleCreate} className="button-primary hover:bg-white mt-4">
        <Plus className="mr-2 h-4 w-4" /> {t("department.create")}
      </Button>

      <Table className="bg-white border border-gray-400 mt-5">
        <TableHeader>
          <TableRow>
            <TableHead className="font-semibold">{t("department.table.name")}</TableHead>
            <TableHead className="font-semibold">{t("department.table.description")}</TableHead>
            <TableHead className="font-semibold">{t("department.table.location")}</TableHead>
            <TableHead className="font-semibold">{t("department.table.establishedDate")}</TableHead>
            <TableHead className="font-semibold">{t("department.table.phoneNumber")}</TableHead>
            <TableHead className="font-semibold">{t("department.table.email")}</TableHead>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-primary-md">
              {currentDepartment ? t("department.dialog.edit") : t("department.dialog.create")}
            </DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-primary-md">
              {t("department.dialog.delete.confirm")}
            </DialogTitle>
            <DialogDescription>
              {t("department.dialog.delete.message")}{" "}
              <span className="font-bold text-black">{departmentToDelete?.name}</span>?{" "}
              {t("department.dialog.delete.warning")}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
            ...
            >
              {t("department.dialog.delete.cancel")}
            </Button>
            <Button className="button-red" variant="destructive" onClick={handleDelete}>
              {t("department.dialog.delete.delete")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

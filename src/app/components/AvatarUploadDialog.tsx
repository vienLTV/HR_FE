  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <button className="button-primary">{t("personal.avatar.change")}</button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="text-primary-md">{t("personal.avatar.title")}</DialogTitle>
          <DialogDescription>{t("personal.avatar.description")}</DialogDescription>
        </DialogHeader>
        ...
        <button onClick={handleUpload} disabled={!file || isUploading} className="button-primary">
          {isUploading ? <Spinner /> : t("personal.avatar.upload")}
        </button>
      </DialogContent>
    </Dialog>
  );

...
export function CreateEmployeeModal({ employeeCreated }: CreateEmployeeModalProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      // Clean up optional fields: remove empty strings so backend doesn't try to parse invalid UUID/Date
      ...
      <DialogContent className="sm:max-w-[600px]">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      ...
                      <SelectContent>
                        {jobTitles.map((job) => (
                          <SelectItem key={job.jobTitleId} value={job.jobTitleId}>
                            {job.title}
                          </SelectItem>
                        ))}
                      </SelectContent>
            ...
            <Button
              className="button-primary bg-white hover:bg-white"
              type="submit"
